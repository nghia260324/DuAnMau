...
public class SachFragment extends Fragment {
    ...
    EditText edt_maSach,edt_tenSach,edt_giaThue,edt_namsb;
    int maLoaiSach = 0;
    int getPosition;
    Button btn_reduce,btn_increase;

    ArrayList<Sach> lstSearch;
    EditText edt_search;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        edt_search.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                String getTextSearch = s.toString().toLowerCase().trim();
                if (getTextSearch.length() != 0) {
                    for (int i = 0; i < lstSach.size(); i++) {
                        if (lstSach.get(i).getTenSach().toLowerCase().contains(getTextSearch)) {
                            lstSearch.add(lstSach.get(i));
                        }
                    }
                    adapter = new SachAdapter(getContext(), lstSearch, new IClickItemRCV() {
                        @Override
    ...
    protected void openDialog(final Context context, final int type) {
        ...
        edt_namsb = dialog.findViewById(R.id.edt_namsb);
        ...
        if (type != 0) {
            ...
            edt_namsb.setText(String.valueOf(sach.getNamSB()));
        ...
        btn_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String namsb = edt_namsb.getText().toString().trim();
                if (validate(tenSach,tienThue,namsb) > 0) {
                    if (type == 0) {
    ...
    public int validate(String tenSach,String giaThue,String namsb) {
        int check;
        if (tenSach.length() != 0 && giaThue.length() != 0 && namsb.length() != 0) {
            sach.setTenSach(tenSach);
            sach.setGiaThue(Integer.parseInt(giaThue));
            sach.setMaLoai(maLoaiSach);
            sach.setNamSB(Integer.parseInt(namsb));
            check = 1;
        } else {
            if (tenSach.length() == 0) {
            ...
            if (namsb.length() == 0) {
                edt_namsb.setError("Không được để trống trường này !");
            }
            check = -1;
        }
        return check;
